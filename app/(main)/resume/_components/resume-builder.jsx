...
export default function ResumeBuilder({ initialContent }) {
  ...
  return;
     ...
     function convertLabToRGB(labStr) {
  // Example lab value: "lab(63.7053% 60.7449 31.3109)"
  const regex = /lab\(([^%]+)%\s+([^\s]+)\s+([^)]+)\)/;
  ...
  // Convert LAB to XYZ then to RGB
  // Simplified, approximate conversion (suitable for fallbacks)

  const y = (L + 16) / 116;
  const x = a / 500 + y;
  const z = y - b / 200;

  const X = 95.047 * Math.pow(x, 3);
  const Y = 100.0 * Math.pow(y, 3);
  const Z = 108.883 * Math.pow(z, 3);
  ...
  bVal = Math.min(Math.max(0, bVal * 255), 255);

  return `rgb(${Math.round(r)}, ${Math.round(g)}, ${Math.round(bVal)})`;
}

function replaceLabColors() {
  ...
  for (const prop of computedStyles) {
    ...
    if (value.includes("lab(")) {
      const rgb = convertLabToRGB(value.trim());
      if (rgb) {
  ...
  // Also fix inline styles or any other computed values on elements
  document.querySelectorAll("*").forEach((el) => {
    const style = getComputedStyle(el);
    ["color", "background-color", "border-color"].forEach((prop) => {
      const val = style.getPropertyValue(prop);
      if (val.includes("lab(")) {
        const rgb = convertLabToRGB(val);
        if (rgb) {
